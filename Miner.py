...
+-------+---------------------------------------+-------+
        | Updated for python 3.6 by Harry Smith |
        +---------------------------------------+
...
def mint(resource, bits=20, now=None, ext='', saltchars=8, stamp_seconds=False):
    ''' PUBLIC Mint a new hashcash stamp for "resource" with "bits" of collision.
    ...
    challenge = "%s:"*6 % (ver, bits, ts, resource, ext, salt(saltchars))
    return challenge + hCChal(challenge,bits)
...
def salt(saltchars):
    ...
    return ''.join([choice(alphabet) for _ in [None]*1])

def hCChal(challenge, bits):
...
def mineBlock(MBC,TX):
    TX.createTree()
    merkelRoot = TX.getRootLeaf()
    txData = TX.printTx()
    currentBlockID = MBC.blockID
    newHashCash = mint(token_hex(64))
        
    MBC.newBlock(newHashCash,merkelRoot,txData)
    print("New block forged")
    
    a = False
    for key in MiniBlockChainV3.MiniBlockChain.blocks.keys():
        if check(MiniBlockChainV3.MiniBlockChain.blocks[key]['hashcash']) == True:
            MiniBlockChainV3.MiniBlockChain.blocks[key]['confirms'] += 1
        else:
            continue
    return True
